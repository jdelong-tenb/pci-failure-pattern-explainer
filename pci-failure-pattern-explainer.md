...
3. **Explain each failure** — plain-language description of what the scanner found and why it is a risk to cardholder data, with a PCI DSS v4.0 requirement cross-reference for each finding. Includes End-of-Life / Unsupported Software mapped to DSS 6.3.3 + 12.3.4 (unsupported software cannot pass an ASV scan regardless of compensating controls).
4. **Rank remediation priority** into three levels: critical path (CVSS ≥ 7.0, must fix), attestation blockers with dispute potential (CVSS ≥ 4.0), and informational. If the workbench result count equals exactly 5,000, the skill flags the likely truncation and recommends filtering by plugin family for complete per-category counts.
